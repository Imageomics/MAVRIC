...
def build_union(source, selected_uuids, dataset):
    source_annotations = {
    ...
    if missing_uuids:
        raise ValueError(
            f"{dataset} has {len(missing_uuids)} selected UUIDs missing from IA35: "
            f"{sorted(missing_uuids)[:5]}"
        )
    ...
    images = [
        image for image in source.get("images", []) if image["uuid"] in image_uuids
    ]
...
def main():
    ...
    if set(by_dataset) != set(dataset_names):
        raise ValueError("Dataset names differ between the suite and task manifest.")

    embedding_rows = []
    for dataset in dataset_names:
        dataset_tasks = by_dataset[dataset]
        if len(dataset_tasks) != 12:
            raise ValueError(f"{dataset} has {len(dataset_tasks)} tasks, expected 12.")

        source_paths = {Path(task["source_json"]) for task in dataset_tasks}
        if len(source_paths) != 1:
            raise ValueError(f"{dataset} has multiple IA35 source files: {source_paths}")
        source_path = source_paths.pop()

        selected_uuids = set()
        for task in dataset_tasks:
            output_path = Path(task["fs_json"])
            if not output_path.is_file():
                raise FileNotFoundError(f"Missing frame-sampling output: {output_path}")
            output = load_json(output_path)
            selected_uuids.update(
                annotation["uuid"] for annotation in output.get("annotations", [])
            )
        if not selected_uuids:
            raise ValueError(f"{dataset} has no selected annotations across the sweep.")

        source = load_json(source_path)
        union = build_union(source, selected_uuids, dataset)
        experiment_root = source_path.parent.parent
        union_dir = experiment_root / f"{dataset}_IA_threshold_embedding_union"
        ...
        embedding_rows.append(
            {
                ...
                "model_url": MODEL_URL,
                "embedding_pickle": str(
                    union_dir / "miewid_union_embeddings.pickle"
                ),
                "run_log": str(union_dir / "miewid_union.log"),
                "num_annotations": len(union["annotations"]),
            }
        )
    ...
    print(f"Built {len(embedding_rows)} IA-threshold embedding unions.")
    print(
        "Total unique annotations: "
        f"{sum(int(row['num_annotations']) for row in embedding_rows)}"
    )
